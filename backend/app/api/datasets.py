# Gamur dataset API — production-safe Supabase operations
...
def _profile_file(data: bytes, suffix: str) -> dict:
    try:
        ...
        raise HTTPException(status_code=400, detail="The uploaded file could not be parsed as a valid dataset") from exc
    if not result.valid:
        raise HTTPException(status_code=422, detail={"message": "Dataset failed validation", "errors": result.errors})
...
def _ingest_version(dataset_id: UUID, project_id: UUID, user_id: str, upload: UploadFile, supabase: Client) -> DatasetVersionResponse:
    ...
    row = {
        "id": str(version_id), "dataset_id": str(dataset_id), "version_no": version_no,
        "original_filename": safe_name, "storage_path": storage_path, "mime_type": upload.content_type,
        "size_bytes": len(data), "sheet_name": profile.get("sheet_name"), "detected_header_row": 0,
        "row_count": profile["row_count"], "column_count": profile["column_count"],
        "quality_score": profile["quality_score"], "profile": profile,
    }
    try:
        supabase.storage.from_(_STORAGE_BUCKET).upload(storage_path, data, {
            "content-type": upload.content_type or "application/octet-stream", "upsert": "false",
        })
        ...
        supabase.table("datasets").update({"current_version": version_no}).eq("id", str(dataset_id)).execute()
    except Exception as exc:
        try:
...
def create_dataset(project_id: UUID, file: UploadFile = File(...), name: str = Form(..., min_length=1, max_length=200), description: str | None = Form(default=None, max_length=2000), user_id: str = Depends(get_current_user_id), supabase: Client = Depends(get_supabase_client)):
    ...
    profile = _profile_file(data, suffix)
    dataset_id = uuid4(); version_no = 1
    safe_name = PurePosixPath(file.filename or f"dataset{suffix}").name
    storage_path = f"{user_id}/{project_id}/{dataset_id}/{version_no}/{safe_name}"; version_id = uuid4()
    dataset_row = {"id": str(dataset_id), "project_id": str(project_id), "name": clean_name, "source_type": "upload", "description": description, "current_version": version_no}
    version_row = {
        "id": str(version_id), "dataset_id": str(dataset_id), "version_no": version_no,
        "original_filename": safe_name, "storage_path": storage_path, "mime_type": file.content_type,
        "size_bytes": len(data), "sheet_name": profile.get("sheet_name"), "detected_header_row": 0,
        "row_count": profile["row_count"], "column_count": profile["column_count"],
        "quality_score": profile["quality_score"], "profile": profile,
    }
    try:
        ...
        supabase.storage.from_(_STORAGE_BUCKET).upload(storage_path, data, {"content-type": file.content_type or "application/octet-stream", "upsert": "false"})
...
def list_datasets(project_id: UUID, user_id: str = Depends(get_current_user_id), supabase: Client = Depends(get_supabase_client)):
    _require_project(project_id, user_id, supabase)
    try:
        response = supabase.table("datasets").select("*").eq("project_id", str(project_id)).order("created_at", desc=True).execute()
    except Exception as exc:
        _database_error(exc)
