...
export default function WorkspaceProviderFixed() {
  ...
  const analysisContext = data?.analysis_context || intelligence?.analysis_context;
  ...
  async function ask() {
    ...
    try {
      const r = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: q, context: JSON.stringify({ dataset, computed_analysis: analysisContext, intelligence }), language: "en", provider: provider === "auto" ? undefined : provider }) });
      const body = await r.json().catch(() => ({}));
