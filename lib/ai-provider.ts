...
function safeError(provider: string, status: number, body: string) { return `${provider} ${status}: ${body.slice(0, 500)}`; }
...
async function anthropic(instructions: string, input: string) {
  ...
  const r = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json", "x-api-key": key, "anthropic-version": "2023-06-01" }, body: JSON.stringify({ model: process.env.ANTHROPIC_MODEL || "claude-3-5-haiku-latest", max_tokens: 1200, system: instructions, messages: [{ role: "user", content: input }] }) });
...
async function kimi(instructions: string, input: string) {
  ...
  if (!r.ok) throw new Error(safeError("Kimi", r.status, await r.text()));
